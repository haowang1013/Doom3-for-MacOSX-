...
int idWindow::GetWinVarOffset( idWinVar *wv, drawWin_t* owner) {
	...
	if ( wv == &rect ) {
		ret = (int64_t)&( ( idWindow * ) 0 )->rect;
	}

	if ( wv == &backColor ) {
		ret = (int64_t)&( ( idWindow * ) 0 )->backColor;
	}

	if ( wv == &matColor ) {
		ret = (int64_t)&( ( idWindow * ) 0 )->matColor;
	}

	if ( wv == &foreColor ) {
		ret = (int64_t)&( ( idWindow * ) 0 )->foreColor;
	}

	if ( wv == &hoverColor ) {
		ret = (int64_t)&( ( idWindow * ) 0 )->hoverColor;
	}

	if ( wv == &borderColor ) {
		ret = (int64_t)&( ( idWindow * ) 0 )->borderColor;
	}

	if ( wv == &textScale ) {
		ret = (int64_t)&( ( idWindow * ) 0 )->textScale;
	}

	if ( wv == &rotate ) {
		ret = (int64_t)&( ( idWindow * ) 0 )->rotate;
	}
...
void idWindow::FixupTransitions() {
	...
	for ( i = 0; i < c; i++ ) {
		...
		if ( dw && ( dw->win || dw->simp ) ){
			if ( dw->win ) {
				if ( transitions[i].offset == (int64_t)&( ( idWindow * ) 0 )->rect ) {
					transitions[i].data = &dw->win->rect;
				} else if ( transitions[i].offset == (int64_t)&( ( idWindow * ) 0 )->backColor ) {
					transitions[i].data = &dw->win->backColor;
				} else if ( transitions[i].offset == (int64_t)&( ( idWindow * ) 0 )->matColor ) {
					transitions[i].data = &dw->win->matColor;
				} else if ( transitions[i].offset == (int64_t)&( ( idWindow * ) 0 )->foreColor ) {
					transitions[i].data = &dw->win->foreColor;
				} else if ( transitions[i].offset == (int64_t)&( ( idWindow * ) 0 )->borderColor ) {
					transitions[i].data = &dw->win->borderColor;
				} else if ( transitions[i].offset == (int64_t)&( ( idWindow * ) 0 )->textScale ) {
					transitions[i].data = &dw->win->textScale;
				} else if ( transitions[i].offset == (int64_t)&( ( idWindow * ) 0 )->rotate ) {
					transitions[i].data = &dw->win->rotate;
				}
			} else {
				if ( transitions[i].offset == (int64_t)&( ( idSimpleWindow * ) 0 )->rect ) {
					transitions[i].data = &dw->simp->rect;
				} else if ( transitions[i].offset == (int64_t)&( ( idSimpleWindow * ) 0 )->backColor ) {
					transitions[i].data = &dw->simp->backColor;
				} else if ( transitions[i].offset == (int64_t)&( ( idSimpleWindow * ) 0 )->matColor ) {
					transitions[i].data = &dw->simp->matColor;
				} else if ( transitions[i].offset == (int64_t)&( ( idSimpleWindow * ) 0 )->foreColor ) {
					transitions[i].data = &dw->simp->foreColor;
				} else if ( transitions[i].offset == (int64_t)&( ( idSimpleWindow * ) 0 )->borderColor ) {
					transitions[i].data = &dw->simp->borderColor;
				} else if ( transitions[i].offset == (int64_t)&( ( idSimpleWindow * ) 0 )->textScale ) {
					transitions[i].data = &dw->simp->textScale;
				} else if ( transitions[i].offset == (int64_t)&( ( idSimpleWindow * ) 0 )->rotate ) {
